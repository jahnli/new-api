...
AI Gateway 是面向应用、Agent 和团队的自托管 AI 网关。将不同厂商的模型服务接入统一入口，在同一套控制台中管理渠道、访问权限、用量与成本。
...
| 视频 | [视频 API 文档](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
...
打开 [http://localhost:3002](http://localhost:3002)，按初始化向导创建管理员账号。SQLite 数据库存放在挂载的 `data` 目录中，更换容器后仍会保留。
...
4. 对于 OpenAI 兼容客户端，将 Base URL 设为 `http://localhost:3002/v1`，密钥使用 **AI Gateway 签发的 Key**。

在终端中将 `AI_GATEWAY_KEY` 环境变量设为该密钥，查询它可以访问的模型：
...
仓库的 [Compose 配置](./docker-compose.yml) 默认启动 **AI Gateway + PostgreSQL + Redis**，并提供 MySQL 和独立 ClickHouse 日志库的配置示例。

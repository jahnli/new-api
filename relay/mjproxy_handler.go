...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if midjResponse.Code == 3 {
		...
		if err != nil {
		...
		} else if channel != nil && channel.GetAutoBan() && common.AutomaticDisableChannelEnabled {
			...
			service.DisableChannel(*channelError, "No available account instance")
			service.RequestPolicy(c).AddEvent(service.PolicyEvent{
				ChannelID: midjourneyTask.ChannelId,
				Decision:  service.PolicyDecision{Action: "stop", Reason: "task_failed", Source: "upstream"},

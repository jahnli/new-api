...
func TestHandleGroupRatioUsesBaseModelVendorForModifiers(t *testing.T) {
	previousDB := model.DB
	...
	originalVendorRatios := ratio_setting.GroupVendorRatio2JSONString()
	t.Cleanup(func() {
		require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(originalGroupRatios))
		require.NoError(t, ratio_setting.UpdateGroupGroupRatioByJSONString(originalSpecialRatios))
		require.NoError(t, ratio_setting.UpdateGroupVendorRatioByJSONString(originalVendorRatios))
	...
	vendor := model.Vendor{Name: "modifier-vendor", Status: 1}
	require.NoError(t, db.Create(&vendor).Error)
	require.NoError(t, db.Create(&model.Model{
		ModelName: "modifier-vendor-model",
		VendorID:  vendor.Id,
		Status:    1,
	...
	vendorRatios, err := common.Marshal(map[string]map[string]float64{
		"default": {fmt.Sprint(vendor.Id): 0.4},
	})
	require.NoError(t, err)
	require.NoError(t, ratio_setting.UpdateGroupVendorRatioByJSONString(string(vendorRatios)))

	ctx, _ := gin.CreateTestContext(httptest.NewRecorder())
	info := &relaycommon.RelayInfo{
		OriginModelName: "modifier-vendor-model@effort:high",
		UserGroup:       "default",
		UsingGroup:      "default",
	}
	...
	assert.True(t, groupRatio.HasVendorRatio)
	assert.Equal(t, vendor.Id, groupRatio.VendorID)
	assert.Equal(t, 0.4, groupRatio.GroupRatio)
}
...
func TestModelPriceHelperRequestBillingRatiosApplyToFixedAndRatioPrices(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{
			name:           "ratio price applies image count",

...
func HandleGroupRatio(ctx *gin.Context, relayInfo *relaycommon.RelayInfo) hosttypes.GroupRatioInfo {
	...
	// 特殊倍率 > 分组供应商倍率 > 分组基础倍率
	vendorModelName := relayInfo.OriginModelName
	vendorID, found := model.GetModelVendorID(vendorModelName)
	if !found {
		vendorModelName = ratio_setting.RoutingMatchModelName(vendorModelName)
		vendorID, _ = model.GetModelVendorID(vendorModelName)
	}
	return ratio_setting.ResolveGroupRatio(relayInfo.UserGroup, relayInfo.UsingGroup, vendorID)
}
...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	...
	if !usePrice {
		...
		billingRatios := hosttypes.PriceData{}
		for name, billingRatio := range meta.BillingRatios {
			billingRatios.AddOtherRatio(name, billingRatio)
		}
		quotaToPreConsume := billingRatios.ApplyOtherRatiosToFloat(float64(preConsumedTokens) * ratio)

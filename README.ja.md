...
AI Gateway は、アプリケーション、Agent、チーム向けのセルフホスト型 AI ゲートウェイです。各社のモデルサービスを共通の入口に接続し、ルーティング、アクセス権、使用量、コストを一つの管理画面で扱えます。
...
| 動画 | [動画 API ドキュメント](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
...
[http://localhost:3002](http://localhost:3002) を開き、初期設定ウィザードで管理者アカウントを作成します。SQLite データベースはマウントした `data` ディレクトリに保存され、コンテナを置き換えても残ります。
...
4. OpenAI 互換クライアントでは Base URL を `http://localhost:3002/v1` に設定し、**AI Gateway が発行したキー**を使います。

シェルの `AI_GATEWAY_KEY` にそのキーを設定し、アクセス可能なモデルを確認します。
...
リポジトリの [Compose 設定](./docker-compose.yml) は、既定で **AI Gateway + PostgreSQL + Redis** を起動します。MySQL と独立した ClickHouse ログデータベースの設定例も含まれます。

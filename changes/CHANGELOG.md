...
| 039 | 2026-09-11 | 用户管理增强：完善用户表格头像、资料、月度统计、服务端排序、公司筛选、状态人数和共享列；用户编辑弹窗新增可选单成本中心配置，复用完整部门树选择并以单元素部门 JSON 数组持久化，支持创建、编辑、回填、清除、后端校验及七语言文案；成本中心名称保存为不含公司名的完整部门路径，用户 department_name 为空时自动回填该路径；数据总览支持按成本中心归属本地用户（含无 open_id 用户），不依赖平台成员匹配；用户编辑弹窗设置分组时显示各分组基础倍率；用户表格将部门、岗位职级和入职日期合并为紧凑的两行摘要式“任职概况”列，职级与入职日期改用徽章展示，两者均为空时隐藏次行，并为职级预留固定宽度以保持日期对齐；单价列统一按每亿 Token 展示，标题改为“单价 / 亿 Token”、内容改为“金额 / 亿”并加宽列；用户显示名、部门路径与常用模型统一使用常规字重，时间列以图标区分最后登录和创建时间并按该顺序展示，同时补齐七语言文案 | [详情](details/039-user-management-table.md) |
...
| 043 | 2026-09-10 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出和权限裁剪；新增飞书、钉钉、无平台多公司配置与数据隔离；支持 BP 按显式部门配置跨公司查看范围；部门树按权限加载并允许跨公司节点展开；补齐数据总览七语言文案与用户编辑弹窗布局；数据总览日志表格渠道列对超级管理员可见，普通管理员隐藏；详情中的渠道、重试链和分组倍率仅超级管理员可见；统一费用阈值及费用分布中文文案的数字与汉字间距；使用分析四张趋势图支持独立切换日、周、月粒度并在前端聚合；数据总览图表 Token 用量悬浮显示统一为 Token 字面量 + xx.xx 亿格式；用户统计弹窗新增头像+名称展示，仿照请求内容弹窗，支持悬停资料卡片；部门树选择器支持沿禁用祖先节点进入授权部门，保留懒加载导航状态并完善键盘操作与焦点；部门人员列表复用用户管理的“任职概况”和“时间”合并列，用户 ID 与日志页统一为弱化色，页面、加载态、日志弹窗及 Excel 名称统一为“部门人员列表”并补齐七语言文案 | [详情](details/043-data-overview.md) |
...
| 056 | 2026-09-11 | 使用日志与请求内容功能优化：增强用户信息、权限控制、分页筛选、列布局和详情弹框；新增 IP、User-Agent、请求内容及生图 Prompt 展示，支持头像资料卡与飞书跳转；使用日志与审计日志统一保存原始 User-Agent 并限制为 512 个 Unicode 字符，登录审计仅写顶层字段且兼容读取历史重复元数据；请求内容支持参数并排查看、独立滚动、复制和违规通知，相关接口收紧为仅超级管理员可用；请求内容弹框支持一键切换全部展开/收起，并修复重复消息复制时多个按钮同时显示已复制；优化费用、渠道标签、敏感信息显示及管理员「仅自己」视图；安全审计设置响应式布局，非工作时间请求支持违规通知；请求内容弹框折叠触发器改为 div，避免生成原生 button；渠道列和分组倍率等敏感日志信息进一步收紧为仅超级管理员可见；普通日志渠道筛选改为可搜索下拉框，按启用状态、优先级和权重排序，以编号徽章、禁用标签及完整选中态展示，并支持按渠道 ID 精确查询和按渠道名称不区分大小写模糊查询；模型筛选改为可搜索下拉框，管理员聚合渠道模型，普通用户加载可用模型，并保留自定义输入；模型映射详情改为悬浮展示并加宽，长模型名称支持完整换行；演示模式进一步遮蔽费用数字（保留货币符号）、渠道编号与名称、重试链、详情分组倍率和请求内容，令牌列不显示倍率，并阻止请求内容加载与查看；审计日志用户列复用使用日志的头像、显示名/用户名与悬停资料卡 UI，并在首屏批量返回用户展示资料，用户 ID 统一为弱化文字色；使用日志费用金额和令牌徽章统一使用常规字重，IP 地址抽取公共组件供使用日志与审计日志复用并统一常规字重和细描边图标；审计日志与使用日志的时间、用户列头加粗而表格正文保持常规字重，审计详情按钮文字不加粗；审计日志详情弹框与使用日志统一桌面端宽度和内容高度，并优化 User-Agent 内容宽度；将非工作时间与图片安全审计整合进审计日志页面，超级管理员可在统一分类中切换，原独立入口自动跳转兼容，普通用户仍仅访问常规审计 | [详情](details/056-usage-logs-user-column.md) |
...
| 060 | 2026-09-10 | API 密钥界面优化：表格名称列改用截断单元格，创建密钥默认选择用户所属分组，表格与创建/编辑界面的分组倍率仅超级管理员可见；编辑 API 密钥时高级设置默认展开，新建时仍默认收起；IP 限制列右侧新增「快速导入」下拉列（含 CC Switch 选项），将 CC Switch 从三点菜单迁移至新列；CC Switch 导入的官网和 API 端点改用当前页面 Origin，Codex 端点保留 `/v1`；6 语言翻译 | [详情](details/060-api-keys-name-truncate.md) |

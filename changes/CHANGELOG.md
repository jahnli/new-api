...
| 021 | 2026-09-24 | 个人资料页订阅卡片优化：以环形总览展示剩余额度，按标准与高级模型分类展示用量，并在高级额度明细列出当前模型 | [详情](details/021-profile-subscription-card.md) |
...
| 043 | 2026-09-24 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出与权限裁剪，并支持模型系列单价趋势与多公司数据隔离 | [详情](details/043-data-overview.md) |
...
| 046 | 2026-09-24 | 概览页订阅面板优化：展示套餐总额度，并按标准与高级模型拆分用量、占比和分阶段进度 | [详情](details/046-overview-subscription-summary.md) |
...
| 049 | 2026-09-23 | 模型定价编辑器支持本地货币输入及超级管理员跨设备同步货币偏好，保留价格精度与档位名称原样回显 | [详情](details/049-model-pricing-local-currency.md) |
...
| 056 | 2026-09-24 | 使用日志与审计日志优化：统一用户信息、权限、筛选与详情展示，支持多模型筛选、Token 分类汇总、请求内容审计、订阅来源展示、额度调整标记及按当前筛选条件直接导出 Excel；导出增加日期-渠道(模型)标题、记录与费用汇总、筛选范围及统计口径说明，统一文件名并按参考脚本拆分缓存 Token | [详情](details/056-usage-logs-user-column.md) |

...
| 037 | 2026-09-01 | 订阅管理增强：全员订阅按公司覆盖，管理员可按人民币金额增减用户总额度，额度重置仅当期生效，套餐按公司限制可见范围 | [详情](details/037-subscribe-all-users.md) |
...
| 043 | 2026-09-10 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出与权限裁剪；新增飞书、钉钉、无平台多公司配置与数据隔离，支持 BP 按显式部门配置跨公司查看；部门树按权限加载，部门人员列表展示统一 | [详情](details/043-data-overview.md) |
...
| 055 | 2026-09-13 | 模型广场与分组定价优化：新增分组 × 供应商倍率配置与统一计费优先级、定价响应加密、超级管理员「模型广场配置」入口、自动循环的顶部推荐模型横滑区及模型卡片悬停边框 | [详情](details/055-pricing-square.md) |
| 056 | 2026-09-13 | 使用日志、审计日志与请求内容弹框全面优化：补齐用户信息与 IP/User-Agent/请求内容展示，收紧渠道、倍率等敏感信息为仅超级管理员可见，统一筛选下拉框、弹框布局与共用用户身份组件，并调整配套列宽、字重与色彩 | [详情](details/056-usage-logs-user-column.md) |
...
| 064 | 2026-07-25 | 新增 AI 中转站周报统计脚本：按使用日志计费快照拆分输入/输出/缓存 Token 与费用，汇总均价、缓存命中率及费用 Top 5 模型 | [详情](details/064-weekly-stats-script.md) |

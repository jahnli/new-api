...
| 021 | 2026-09-24 | 个人资料页订阅卡片优化：以环形总览和分类用量展示剩余额度，高级额度标题显示占比并在其后提供明细入口，无订阅记录时完整隐藏 | [详情](details/021-profile-subscription-card.md) |
...
| 037 | 2026-09-23 | 订阅管理增强：全员订阅按公司覆盖、管理员可按人民币金额增减总额度或基础/高阶模型额度、额度重置仅当期生效、套餐按公司限制可见范围；订阅与套餐编辑采用居中弹框及人民币额度标签；新增高阶模型额度限制，支持全局模型与默认比例、用户比例覆盖、两位小数比例、额度展示及预扣/结算/退款的周期隔离与幂等记账；高阶额度不足使用专用报错与独立通知，钱包也不足时同时说明两种原因 | [详情](details/037-subscribe-all-users.md) |
...
| 056 | 2026-09-24 | 使用日志与审计日志优化：统一用户信息、权限、筛选与详情展示，支持多模型筛选、Token 汇总、请求内容审计、额度调整标记及按当前筛选条件直接导出 Excel | [详情](details/056-usage-logs-user-column.md) |

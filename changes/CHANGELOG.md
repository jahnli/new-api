...
| 039 | 2026-09-09 | 用户管理增强：完善用户表格头像、资料、月度统计、服务端排序、公司筛选、状态人数和共享列；用户编辑弹窗新增可选单成本中心配置，复用完整部门树选择并以单元素部门 JSON 数组持久化，支持创建、编辑、回填、清除、后端校验及七语言文案；成本中心名称保存为不含公司名的完整部门路径，用户 department_name 为空时自动回填该路径；数据总览支持按成本中心归属本地用户（含无 open_id 用户），不依赖平台成员匹配；用户编辑弹窗设置分组时显示各分组基础倍率 | [详情](details/039-user-management-table.md) |
...
| 043 | 2026-09-07 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出和权限裁剪；新增飞书、钉钉、无平台多公司配置与数据隔离；支持 BP 按显式部门配置跨公司查看范围；部门树按权限加载并允许跨公司节点展开；补齐数据总览七语言文案与用户编辑弹窗布局；数据总览日志表格渠道列对超级管理员可见，普通管理员隐藏；详情中的渠道、重试链和分组倍率仅超级管理员可见；统一费用阈值及费用分布中文文案的数字与汉字间距；使用分析四张趋势图支持独立切换日、周、月粒度并在前端聚合；数据总览图表 Token 用量悬浮显示统一为 Token 字面量 + xx.xx 亿格式；用户统计弹窗新增头像+名称展示，仿照请求内容弹窗，支持悬停资料卡片 | [详情](details/043-data-overview.md) |

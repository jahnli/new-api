...
| 056 | 2026-09-24 | 使用日志与审计日志优化：统一用户信息、权限、筛选与详情展示，支持多模型筛选、Token 分类汇总、请求内容审计、额度调整标记及按当前筛选条件直接导出 Excel；导出增加日期-渠道(模型)标题、记录与费用汇总、筛选范围及统计口径说明，统一文件名并按参考脚本拆分缓存 Token | [详情](details/056-usage-logs-user-column.md) |

...
| 039 | 2026-09-11 | 用户管理增强：完善用户表格头像、资料、月度统计、服务端排序、公司筛选、状态人数和共享列；用户编辑弹窗新增可选单成本中心配置，复用完整部门树选择并以单元素部门 JSON 数组持久化，支持创建、编辑、回填、清除、后端校验及七语言文案；成本中心名称保存为不含公司名的完整部门路径，用户 department_name 为空时自动回填该路径；数据总览支持按成本中心归属本地用户（含无 open_id 用户），不依赖平台成员匹配；用户编辑弹窗设置分组时显示各分组基础倍率；用户表格将部门、岗位职级和入职日期合并为紧凑的两行摘要式“任职概况”列，职级与入职日期改用徽章展示，两者均为空时隐藏次行，并为职级预留固定宽度以保持日期对齐；用户显示名、部门路径与常用模型统一使用常规字重，时间列以图标区分最后登录和创建时间并按该顺序展示，同时补齐七语言文案 | [详情](details/039-user-management-table.md) |

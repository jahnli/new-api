...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		// LDAP login route
		apiRouter.POST("/user/ldap/login", middleware.CriticalRateLimit(), middleware.TurnstileCheck(), controller.LDAPLogin)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				// LDAP bind
				selfRoute.POST("/ldap/bind", middleware.CriticalRateLimit(), controller.LDAPBind)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/:id/stats", controller.GetUserStats)
		...
		subscriptionAdminRoute.Use(middleware.AdminAuth())
		{
			...
			subscriptionAdminRoute.POST("/bind", controller.AdminBindSubscription)
				subscriptionAdminRoute.POST("/batch/bind", controller.AdminBatchBindSubscription)
		...
		// LDAP test connection (root only)
		apiRouter.POST("/ldap/test", middleware.RootAuth(), controller.TestLDAPConnection)
		...
		dataRoute.GET("/self/count", middleware.UserAuth(), controller.GetUserRequestCount)
		dataRoute.GET("/self/consumed", middleware.UserAuth(), controller.GetUserConsumedQuota)
		...
		{
			auditRoute.GET("/security", controller.GetSecurityAuditLogs)
			auditRoute.GET("/security/details", controller.GetSecurityAuditDetails)
		}

...
it.each(['2fa', 'passkey'] as const)(
  'blocks %s enrollment and explains missing OAuth verification',
  async (factor) => {
    const reason = 'No linked OAuth provider is available.'
...
it('refreshes OIDC bindings from the server result after the callback popup has closed', async () => {
  const popup = {
    closed: false,
      ...
      data:
        url === '/api/verify'
          ...
          : {
              ...
              authorization_url:
                'https://idp.example.com/authorize?server=pkce',
            },
    },
  }))
  ...
  const oidc = (await screen.findByText('OIDC')).closest('li')
  if (!oidc) throw new Error('OIDC binding entry is missing')
  await user.click(within(oidc).getByRole('button', { name: 'Bind' }))
  ...
  expect(
    get.mock.calls.find(([url]) => url === '/api/oauth/oidc')?.[1]?.signal
      ?.aborted
  ).toBe(false)
  await act(async () => {

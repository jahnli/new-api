...
it.each([false, true])(
  'retains the OIDC verification request after popup close and honors caller cancellation: %s',
  async (cancel) => {
    const popup = {
      closed: false,
    ...
    vi.spyOn(api, 'post').mockResolvedValue({
      data: {
        ...
        data: {
          ...
          authorization_url: 'https://idp.example.com/authorize?server=pkce',
        },
      },
    })
    const proof: SecurityProof = {
      proof_token: 'oidc-proof',
      method: 'oauth',
      scope: '2fa.setup',

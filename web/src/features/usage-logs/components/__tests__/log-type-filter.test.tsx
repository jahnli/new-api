...
function getTypeCombobox(label: RegExp | string) {
  ...
  if (!trigger) throw new Error('Expected log type combobox')
  return trigger
}
...
it('marks only retired log types as deprecated while keeping historical filters selectable', async () => {
  ...
  await userEvent.click(getTypeCombobox('All Types'))
  ...
  expect(getTypeCombobox('Manage')).toHaveTextContent('Deprecated')
  await userEvent.click(screen.getByRole('button', { name: 'Search' }))
  await waitFor(() =>
    expect(router.state.location.search).toMatchObject({ type: ['3'] })
  )
  await userEvent.click(getTypeCombobox('Manage'))
  await userEvent.click(screen.getByRole('option', { name: /^Login/ }))
  await userEvent.click(screen.getByRole('button', { name: 'Search' }))
  await waitFor(() =>
    expect(router.state.location.search).toMatchObject({ type: ['7'] })
  )
})

  ...
  return (
    ...
    >
      <DataTablePage
        ...
        applyHeaderSize
        tableClassName={cn(
          '[&_[data-slot=table]]:text-[13px] [&_[data-slot=table-body]_td]:font-normal [&_[data-slot=table-body]_td_*]:font-normal [&_[data-slot=table]_td]:text-[13px] [&_[data-slot=table]_td_*]:text-[13px] [&_[data-slot=table]_th]:text-[13px] [&_[data-slot=table]_th_*]:text-[13px]'
        )}
        mobile={
          <UsageLogsMobileList
            table={table}
        ...
        renderRow={(row, helpers) => {
          ...
          if (isCommon && isAdmin) {
            const other = parseLogOther((rowData.other as string) ?? '')

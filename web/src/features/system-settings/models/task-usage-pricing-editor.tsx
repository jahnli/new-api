...
import {
  convertPricingToDisplay,
  convertPricingToUSD,
  formatPricingNumber,
  normalizePricingExchangeRate,
} from './pricing-format'
...
function TaskBillingPreview(props: TaskBillingPreviewProps) {
  ...
  const formulaParts = result.parts.map((part) => {
    if (part.kind === 'constant') {
      return `${props.currencySymbol}${formatPricingNumber(part.amount * props.exchangeRate)}`
    }
    ...
    const quantityLabel =
      definition?.unit === 'second'
        ...
        : `${formatPricingNumber(part.quantity)} ${quantityUnitLabel}`
    return `${quantityLabel} × ${props.currencySymbol}${formatPricingNumber((part.unitPrice ?? 0) * props.exchangeRate)}/${t(priceUnitKey)}`
  })
  const formulaLeft =
    formulaParts.length > 0
      ? formulaParts.join(' + ')
      : `${props.currencySymbol}0`
  const formula = `${formulaLeft} = ${props.currencySymbol}${formatPricingNumber(result.total * props.exchangeRate)}`
...
export const TaskUsagePricingEditor = memo(function TaskUsagePricingEditor(
  ...
  const exchangeRate = normalizePricingExchangeRate(props.exchangeRate)
      ...
      <Alert>
        <AlertDescription className='text-xs'>
          {exchangeRate === 1
            ...
            : t(
                'Task usage prices are entered in {{currency}} and converted to USD when saved. Token fields represent the price per 1M tokens; other fields use their declared unit.',
                          ...
                          <Field key={field} className='gap-1.5'>
                            ...
                            <div className='flex items-center gap-2'>
                              <Input
                                ...
                                min={0}
                                step={0.000001}
                                value={convertPricingToDisplay(
                                  matrixRows[0].unitPrices[field] ?? 0,
                                  exchangeRate
                                )}
                                aria-label={`${field}: ${t('Price')}`}
                                onFocus={(event) => {
                          ...
                          <Input
                            ...
                            onChange={(event) => {
                              ...
                              handleRowChange(0, {
                                ...
                                constant:
                                  Number.isFinite(value) && value >= 0
                                    ? convertPricingToUSD(value, exchangeRate)
                                    : 0,
                              })
                            }}
                            className='font-mono'

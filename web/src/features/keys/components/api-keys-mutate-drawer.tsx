...
export function ApiKeysMutateDrawer({
  ...
  const groups = useMemo<ApiKeyGroupOption[]>(
    ...
    [groupsData, isSuperAdmin]
  )
  const groupOptions = useMemo<ApiKeyGroupOption[]>(
    () =>
      groups.some((group) => group.value === createApiKeyDefaultGroup)
        ? groups
        : [
            {
    ...
    [groups, createApiKeyDefaultGroup]
  )
  ...
  const globalAutoGroups = useMemo(() => {
    const availableGroups = new Set(availableAutoGroupNames)
  ...
  useEffect(() => {
    ...
    if (isUpdate && currentRow) {
      ...
      const shouldUseAutoGroup = defaultUseAutoGroup && backendHasAuto
      form.reset({
        ...getApiKeyFormDefaultValues(shouldUseAutoGroup),
        group: shouldUseAutoGroup ? 'auto' : createApiKeyDefaultGroup,
        cross_group_retry: shouldUseAutoGroup,
      })
      setInitializedTarget(target)
    }
  }, [
  ...
  }, [groups, form, selectedGroup, createApiKeyDefaultGroup])

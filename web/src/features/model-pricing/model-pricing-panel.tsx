...
export function ModelPricingPanel(props: {
  ...
  for (const priceDetail of localPriceDetails) {
    if (
      effectivePricing[priceDetail.field] != null &&
      !details.some((row) => row.key === priceDetail.key)
    ) {
      details.push({
        key: priceDetail.key,
        label: priceDetail.label,
        value: `${formatPrice(effectivePricing, priceDetail.type, 'M')} / 1M`,
      })
    }
  }

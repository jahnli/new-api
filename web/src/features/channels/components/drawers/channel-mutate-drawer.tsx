...
export function ChannelMutateDrawer({
  ...
  const redirectPanelEnabled =
    open && !showProviderPicker && redirectPanelOpen && viewportWide
  const redirectPanelPlacement = useRedirectPanelPlacement(
    channelFormRef,
    redirectPanelEnabled,
    'center'
  )
  const redirectPanelActive =
    redirectPanelEnabled && redirectPanelPlacement !== null
  useEffect(() => {
    setRedirectPanelOpen(false)
    setRedirectSyncModels(true)
    setMappingDraftRequest(null)

...
export function TagBatchEditDialog({
  ...
  const loadKey = open && currentTag ? currentTag : ''
  const queryClient = useQueryClient()
  const [isLoading, setIsLoading] = useState(Boolean(loadKey))
  const [isSaving, setIsSaving] = useState(false)
  const [initializedLoadKey, setInitializedLoadKey] = useState(loadKey)

  if (initializedLoadKey !== loadKey) {
    setInitializedLoadKey(loadKey)

...
const UsersTable = (usersData) => {
  ...
  const [showUserSubscriptionsModal, setShowUserSubscriptionsModal] =
    useState(false);
  const [showUserStatsModalVisible, setShowUserStatsModalVisible] =
    useState(false);
  ...
  const showUserStatsUserModal = (user) => {
    setModalUser(user);
    setShowUserStatsModalVisible(true);
  };
  ...
  const handleTableChange = (changeInfo) => {
    ...
    if (serverSortColumns.has(dataIndex)) {
      if (!newSortOrder) {
        ...
        const sortKey = dataIndex;
        handleSortChange(sortKey, newSortOrder === 'ascend' ? 'asc' : 'desc');
      }
    }
  };
  ...
  const columns = useMemo(() => {
    return getUsersColumns({
      ...
      showUserStatsModal: showUserStatsUserModal,
    });
  }, [
    t,
    setEditingUser,
  ...
  return (
    <>
      ...
      <UserStatsModal
        visible={showUserStatsModalVisible}

...
![AI Gateway](/web/public/logo.png)
...
AI Gateway is a self-hosted AI gateway for applications, agents, and teams. Connect upstream model services, expose a consistent API to your clients, and manage routing, access, usage, and costs in one place.
...
| Video | [Video API documentation](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
...
```bash
mkdir -p data
docker run --name ai-gateway -d --restart unless-stopped \
  -p 127.0.0.1:3002:3002 \
  -e TZ=Asia/Shanghai \
  -e PORT=3002 \
  -v "$(pwd)/data:/data" \
  jahnlis/ai-gateway:latest
```

Open [http://localhost:3002](http://localhost:3002) and complete the setup wizard to create the administrator account. The `data` directory persists the SQLite database across container replacements.
...
4. Set your client's base URL to `http://localhost:3002/v1` for OpenAI-compatible clients and use the **AI Gateway-issued key**.

Set `AI_GATEWAY_KEY` in your shell to that key. List the models accessible to it:

```bash
curl --fail-with-body http://localhost:3002/v1/models \
...
```bash
curl --fail-with-body http://localhost:3002/v1/responses \
  -H "Authorization: Bearer ${AI_GATEWAY_KEY}" \
...
The repository's [Compose configuration](./docker-compose.yml) starts **AI Gateway + PostgreSQL + Redis** by default. It also contains examples for MySQL and a separate ClickHouse log database.

```bash
git clone https://github.com/jahnli/new-api.git
cd new-api
```
...
See the [environment example](./.env.example), [environment reference](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c), and [authentication and session guide](./docs/authentication.md) for full configuration. Configure container variables in Compose's `environment` or `env_file`; copying `.env.example` alone does not inject variables into the container.
...
| Resource | Link |
| --- | --- |
| Official documentation | [Guides](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) · [Installation](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) · [API reference](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
| Project exploration | [DeepWiki](https://deepwiki.com/QuantumNous/new-api) |
| Questions and discussion | [FAQ](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) · [Community](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
| Bugs and feature requests | [Issue feedback](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c) |
...
### 💖 Thank you for using AI Gateway

If this project is helpful to you, welcome to give us a ⭐️ Star！

**[Official Documentation](https://semi-tech.feishu.cn/docx/GhoWd4iMookr4BxbBXcc2Qudn2c)** • **[Issue Feedback](https://github.com/Calcium-Ion/new-api/issues)** • **[Latest Release](https://github.com/Calcium-Ion/new-api/releases)**

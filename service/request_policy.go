...
func RecordPolicyFailure(c *gin.Context, channelID int, err *types.AIGatewayError, decision PolicyDecision) {
	if c == nil || err == nil {
		return
	}
	source := "upstream"
	...
	case err.GetErrorCode() == types.ErrorCodeDoRequestFailed:
		source = "transport"
	case err.GetErrorType() == types.ErrorTypeAIGatewayError:
		source = "local"
	}
...
func RecordRequestPolicyTermination(c *gin.Context, apiErr *types.AIGatewayError) {
	if c == nil || apiErr == nil {
		return
	}
	state := RequestPolicy(c)

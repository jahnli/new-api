...
func subscriptionPremiumQuotaError(_ bool) *types.AIGatewayError {
	const message = "高级模型额度不足，标准模型仍可使用剩余订阅额度。可等待额度重置或通过飞书、钉钉申请高级模型额度"
	return types.NewErrorWithStatusCode(errors.New(message), errorCodeSubscriptionPremiumQuotaInsufficient, http.StatusPaymentRequired,

		...
		t.Run(fmt.Sprintf("direct=%t", direct), func(t *testing.T) {
			...
			for _, verified := range []bool{false, true} {
				...
				if direct {
					...
					pending, err := service.StartLoginVerification(user, "oauth:oidc")
					require.NoError(t, err)
...
func (*boundLoginOAuthProvider) ProviderUserIDColumn() string { return "oidc_id" }
...
func TestSecurityLoginAllPrimaryTransportsRequireAdditionalVerification(t *testing.T) {
	for _, transport := range []string{"oauth", "custom oauth", "wechat"} {
...
func TestSecurityLoginPasskeyCannotCompleteAnotherChallenge(t *testing.T) {
	for _, otherUser := range []bool{false, true} {
		t.Run(fmt.Sprintf("other-user=%t", otherUser), func(t *testing.T) {
			...
			if otherUser {
				user = &model.User{Username: "other-login", Role: common.RoleCommonUser, Status: common.UserStatusEnabled, Group: "default", AuthVersion: 1}
...
func TestGenerateOAuthCodeIgnoresRemovedAffiliateField(t *testing.T) {

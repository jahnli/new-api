		...
		t.Run(scenario, func(t *testing.T) {
			...
			case "other session", "other account":
				...
				if scenario == "other account" {
					other := &model.User{Username: "other", Group: "default", Password: user.Password, Status: common.UserStatusEnabled, Role: common.RoleCommonUser, AuthVersion: 1}
...
func TestSecurityAccountEmailConcurrentClaimsHaveOneOwner(t *testing.T) {
	...
	mailbox := newSecurityMailbox(t)
	other := &model.User{Username: "other-user", Group: "default", Password: user.Password, Status: common.UserStatusEnabled, Role: common.RoleCommonUser, AuthVersion: 1}
	require.NoError(t, model.DB.Create(other).Error)
...
func TestSecurityAccountOAuthBindingRejectsInvalidFlow(t *testing.T) {
	for _, scenario := range []string{"success and replay", "other session", "wrong provider", "expired", "old flow", "mfa enabled"} {
		t.Run(scenario, func(t *testing.T) {
			...
			current, err := model.GetUserById(user.Id, false)
			require.NoError(t, err)
			assert.Equal(t, scenario == "success and replay", current.OidcId == "new-binding")
			if result.Success {

...
func TestSecurityEnrollmentAccessTokenMethodPolicy(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.oauth {
				require.NoError(t, model.DB.Model(user).Update("oidc_id", "linked-user").Error)
	...
	for _, method := range []string{service.VerificationMethodPasskey, service.VerificationMethodOAuth} {
		t.Run(method, func(t *testing.T) {
			...
			if method == service.VerificationMethodPasskey {
				...
				require.NoError(t, model.DB.Model(user).Updates(map[string]any{"password": "", "oidc_id": "linked-user"}).Error)
				oauth.Register("enrollment-oauth", &enrollmentOAuthProvider{externalID: "linked-user"})
				t.Cleanup(func() { oauth.Unregister("enrollment-oauth") })
			}
...
func (*enrollmentOAuthProvider) ProviderUserIDColumn() string { return "oidc_id" }
...
func TestSecurityEnrollmentOAuthVerificationNeverChangesLoginOrBindings(t *testing.T) {
	for _, scenario := range []string{"success", "different account", "binding changed", "session changed", "auth version changed", "provider disabled", "cancelled"} {
		t.Run(scenario, func(t *testing.T) {
			user, identity := setupSecurityEnrollmentTest(t)
			require.NoError(t, model.DB.Model(user).Updates(map[string]interface{}{"password": "", "oidc_id": "linked-user", "email": "same@example.com"}).Error)
			...
			case "binding changed":
				require.NoError(t, model.DB.Model(user).Update("oidc_id", "replacement-user").Error)
			...
			assert.Equal(t, expectedBinding, stored.OidcId)
		})
	}
}
...
func TestSecurityEnrollmentOIDCAndWeChatFirstFactor(t *testing.T) {
	for _, provider := range []string{"oidc", "wechat"} {
		for _, scope := range []string{service.VerificationScopeTwoFASetup, service.VerificationScopePasskeyRegister} {
			t.Run(provider+"/"+scope, func(t *testing.T) {
				...
				if provider == "oidc" {
					require.NoError(t, model.DB.Model(user).Updates(map[string]any{"password": "", "oidc_id": "linked-user"}).Error)
					oauthProvider := &enrollmentOAuthProvider{externalID: "other-user"}
					oauth.Register("enrollment-oidc", oauthProvider)
					t.Cleanup(func() { oauth.Unregister("enrollment-oidc") })
					for _, externalID := range []string{"other-user", "linked-user"} {
						oauthProvider.externalID = externalID
						request, err := common.Marshal(map[string]string{"provider": "enrollment-oidc", "intent": "verify", "scope": scope})
						require.NoError(t, err)
						started := securityEnrollmentRequest("POST", "/api/oauth/state", string(request), "", identity, GenerateOAuthCode)
						var startedBody securityEnrollmentResponse
						require.NoError(t, common.Unmarshal(started.Body.Bytes(), &startedBody))
						require.True(t, startedBody.Success, started.Body.String())
						var flow struct {
							FlowToken string `json:"flow_token"`
						}
						require.NoError(t, common.Unmarshal(startedBody.Data, &flow))
						handler := func(c *gin.Context) {
							c.Params = gin.Params{{Key: "provider", Value: "enrollment-oidc"}}
							HandleOAuth(c)
						}
						callback := securityEnrollmentRequest("GET", "/api/oauth/enrollment-oidc?state="+flow.FlowToken+"&code=authorization-code", "", "", identity, handler)
						if externalID == "other-user" {
							assert.Contains(t, callback.Body.String(), "OAUTH_ACCOUNT_MISMATCH")
							assert.NotContains(t, callback.Body.String(), "proof_token")
							continue
						}
						response = callback
					}
				...
				require.NoError(t, err)
				assert.Equal(t, before.OidcId, after.OidcId)
...
func TestSecurityEnrollmentNeverTrustsSessionForFirstFactor(t *testing.T) {
	for _, scenario := range []string{"wechat only", "password", "passkey", "locked 2fa", "oidc", "disabled custom binding", "no binding", "binding storage failure", "revoked session"} {
		t.Run(scenario, func(t *testing.T) {
			user, identity := setupSecurityEnrollmentTest(t)
			require.NoError(t, model.DB.Model(user).Updates(map[string]any{"password": "", "wechat_id": "wechat-user"}).Error)
			switch scenario {
			...
			case "oidc":
				require.NoError(t, model.DB.Model(user).Update("oidc_id", "42").Error)
...
func TestSecurityEnrollmentRejectsChangedFirstFactorPolicy(t *testing.T) {
	for _, stage := range []string{"proof", "setup"} {
		t.Run("oidc/"+stage, func(t *testing.T) {
			user, identity := setupSecurityEnrollmentTest(t)
			require.NoError(t, model.DB.Model(user).Updates(map[string]any{"password": "", "oidc_id": "linked-user"}).Error)
			provider := &enrollmentOAuthProvider{externalID: "linked-user"}
			oauth.Register("policy-oidc", provider)
			t.Cleanup(func() { oauth.Unregister("policy-oidc") })

			request, err := common.Marshal(map[string]string{"provider": "policy-oidc", "intent": "verify", "scope": "2fa.setup"})
			require.NoError(t, err)
			started := securityEnrollmentRequest("POST", "/api/oauth/state", string(request), "", identity, GenerateOAuthCode)
			var startedBody securityEnrollmentResponse
			require.NoError(t, common.Unmarshal(started.Body.Bytes(), &startedBody))
			...
			require.NoError(t, common.Unmarshal(startedBody.Data, &flow))
			handler := func(c *gin.Context) {
				c.Params = gin.Params{{Key: "provider", Value: "policy-oidc"}}
				HandleOAuth(c)
			}
			response := securityEnrollmentRequest("GET", "/api/oauth/policy-oidc?state="+flow.FlowToken+"&code=authorization-code", "", "", identity, handler)
			var body securityEnrollmentResponse
			require.NoError(t, common.Unmarshal(response.Body.Bytes(), &body))
			require.True(t, body.Success, response.Body.String())
			var proof service.SecurityProof
